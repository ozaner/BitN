    ...
    where TSelf : IBitNUInt32Sized<TSelf>
{
    ...
    {
        value = TSelf.Zero;
        if (BitNUtil.OutOfBounds(source.Length, sizeof(uint), TSelf.BitCount, offset)) return false;
    ...
    {
        if (BitNUtil.OutOfBounds(destination.Length, sizeof(uint), TSelf.BitCount, offset)) return false;
    ...
    where TSelf : IBitNUInt16Sized<TSelf>
{
    ...
    {
        value = TSelf.Zero;
        if (BitNUtil.OutOfBounds(source.Length, sizeof(ushort), TSelf.BitCount, offset)) return false;
    ...
    {
        if (BitNUtil.OutOfBounds(destination.Length, sizeof(ushort), TSelf.BitCount, offset)) return false;
    ...
    where TSelf : IBitNByteSized<TSelf>
{
    ...
    {
        value = TSelf.Zero;
        if (BitNUtil.OutOfBounds(source.Length, sizeof(byte), TSelf.BitCount, offset)) return false;
        value = TSelf.ReadByteBacked(source, offset);
        return true;
    }
    bool TryWriteByteBacked(Span<byte> destination, int offset)
    {
        if (BitNUtil.OutOfBounds(destination.Length, sizeof(byte), TSelf.BitCount, offset)) return false;
